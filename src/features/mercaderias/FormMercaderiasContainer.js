...
function mapStateToProps(state) {
  ...
  for (let subGrupo of state.mercaderiasSubGrupos.subGrupos) {
    if (
      (modoNuevo && subGrupo.b_activo) ||
      ((!modoNuevo && initialValues.n_id_subgrupo === subGrupo.id) || subGrupo.b_activo)
    ) {
      subGrupoObj = {
  ...
  for (let marca of state.mercaderiasMarcas.marcas) {
    if (
      (modoNuevo && marca.b_activo) ||
      ((!modoNuevo && initialValues.n_id_marca === marca.id) || marca.b_activo)
    ) {
      marcasObj = {
