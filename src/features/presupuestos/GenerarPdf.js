...
const Seccion50Total = styled.View `
  height: 25px;
  width: 42%;
  border: 1px solid #000;
  border-left: 0;
...
const Seccion30Total = styled.View `
  width: 30%;
  height: 25px;
  border: 1px solid #000;
  padding-top: 5px;
        ...
        <Document title={archivo} author={props.configuracion.c_razon_social}>
            <Page size="A4" style={styles.page}>
                <Linea50>
                    <SeccionLogo>
                        {props.blob
                            ? <Image src={props.blob} style={styles.logo}/>
                            : null}
                    </SeccionLogo>
                    <Seccion1>
                        <Campo>Empresa:</Campo>
                ...
                <Linea>
                    <SeccionSlogan>
                        <Campo8 style={styles.negrito}>{props
                                .configuracion
                                .c_slogan
                                .toUpperCase()}</Campo8>
                    </SeccionSlogan>
                ...
                <Linea35>
                    ...
                    <ClienteValor>
                        <Valor8Izquierda>{`${props
                                ...
                                .presupuesto
                                .persona
                                .c_identificacion}`}</Valor8Izquierda>
                ...
                {props
                    ...
                    .map((item, indice) => (
                        <Linea key={indice}>
                            <SeccionDescValor>
                                <Campo8Izquierda>{item
                                        .c_descripcion
                                        .toUpperCase()}</Campo8Izquierda>
                            </SeccionDescValor>
                            <SeccionCant>
                ...
                <Linea>
                    ...
                    <Seccion10>
                        <Valor8>{formatarNumero(props.presupuesto.n_total_flete, decimales, true)}</Valor8>
                    </Seccion10>
                    <Seccion10>
                ...
                <Linea>
                    ...
                    <Seccion50Total>
                        <Campo8Derecha>(Flete + Total Items + Comisión + Seguro + Descuento/Redondeo)</Campo8Derecha>
                    </Seccion50Total>
                    <Seccion30Total>
...
// const pdf_ios = (blob) => { var fileBlob = new Blob([blob], {type:
// 'application/pdf'}); window.open(fileBlob, '_blank'); return true; }
...
export class GenerarPdf extends Component {
    ...
    descargaInfos = idPresupuesto => {
        ...
        api_axio({api_funcion: `presupuestos/${idPresupuesto}`}).then(res => {
            this.setState(state => ({presupuesto: res.data}));
        });
        api_axio({api_funcion: `presupuestos/cuotas/${idPresupuesto}`}).then(res => {
            this.setState(state => ({cuotas: res.data}));
            api_axio({api_funcion: `presupuestos/itemsMercaderiasServicios/${idPresupuesto}`}).then(res => {
    ...
    render() {
        ...
        const cargado = Object
            .keys(items)
            .length > 0 && Object
            .keys(presupuesto)
            .length > 0 && img;
        const archivo = cargado
            ? `presupuesto_${presupuesto
                .id}_${presupuesto
                .persona
                .c_nombre
                .replace(/[^a-z0-9]/gi, '_')
...
function mapStateToProps(state) {
    return {configuracion: state.configuraciones.configuracion};
}
...
export default connect(mapStateToProps, mapDispatchToProps,)(GenerarPdf);
