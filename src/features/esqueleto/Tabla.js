...
import {procesarTabla, lineaSeleccionada, modalToggle, limpiaImg, limpiarItemLinea} from './redux/actions';
...
const sinPrivilegios = () => {
    swal({title: 'Ops', text: 'No tienes privilegios para esta acción', icon: 'warning', button: 'OK!'});
};
...
const RemoteAll = ({
    ...
    const editar = () => {
        if (usuario.b_editar) {
            if (selected.length === 0) {
                swal({title: 'Ops', text: 'Debes seleccionar una linea primero', icon: 'warning', button: 'OK!'});
            } else {
                sinModal
            ...
            if (selected.length === 0) {
                ...
                swal({
                    ...
                    if (willDelete) {
                        ...
                        return api_axio({api_funcion, params}).then(res => {
                            procesarTabla({
                                api_funcion,
                                offset: 0,
                                sizePerPage,
    ...
    const beforeSaveCell = (oldValue, newValue, row, column, done) => {
        ...
        api_axio({api_funcion, params}).then(res => {
            swal({icon: 'success', timer: 1000});
            done(true);
        }).catch(err => {
                        ...
                        {({paginationProps, paginationTableProps}) => (
                            <div>
                                ...
                                <BootstrapTable
                                    ...
                                    selectRow={selectRow}
                                    cellEdit={cellEditFactory({mode: 'dbclick', blurToSave: true, autoSelectText: true, beforeSaveCell})}
